...
3. **No-operator block.** Every operator-gated skill routes to the no-operator protocol (`workflow.md`) with the canonical `**No operator to ask?**` line — in the intro, or at the gate it governs where that reads better (`import-vendor-asset` and `shoot-thumbnail` place it at the gate, each with the skill's derivable default). A skill that runs unattended by design declares itself in the constants' `autonomous_skills` instead of carrying the block.
...
autonomous_skills: [showcase-record]      # run unattended by design: no no-operator block owed
