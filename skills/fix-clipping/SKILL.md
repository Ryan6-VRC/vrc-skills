...
5. **Verify.** Re-run `ReportClearance`: `restContactCm=0` on every chain it lists. Then the play-mode harness ([references/play-penetration-probe.md](references/play-penetration-probe.md)), component off then on, for the motion number. Pass: depth reduced on every named chain and zero rest contact. Report the residual as a number, not as "fixed".
