...
**A texture-mask cut is the reversible alternative**: an MA `MeshCutter` carrying `Vertex Filter - By Mask` components deletes the triangles an exact-black (or exact-white) mask region covers, at build, with no Blender round-trip. Set each filter's selection mode to `AllVertices`; its default `AnyVertex` takes every triangle with one vertex in the region, visible edge triangles included. A filter reads one material slot and the cutter's default `VertexIntersection` ANDs its filters per slot, so filters on different slots cut nothing: intersect only filters on one slot, one per material that can occupy it. The cut reads only the mask, the mesh's UVs and the slot index, never the material, so a mask traced from one material's transparency is wrong for any other material a swap can put in that slot: give each such material its own filter.
...
3. **Choose, reversible first.**
   - **Size the pair together: chain radii large, body colliders shrunk to match.** A skirt chain's radius must be large enough that no leg-sized gap opens between neighbouring chains, and as the chains grow the body colliders shrink in step, so their rest contact stays a touch rather than a deflection. Resize the collider already on the bone the body region rides before adding one. The operator's heuristics, quoted: "if the individual chain colliders are small enough to leave a leg-sized gap between them, a leg will slip between. And behavior is almost always better when using larger colliders, assuming they do not touch at rest" — rest contact tolerated, a major rest deflection not — "skirt chain colliders need to be very large because otherwise legs can slip between the chains. But as the skirt chains become larger, the body colliders have to shrink", and "On a single-sheet like an apron you want to err towards too much space, since clipping is just immediately bad. On frilly/layered skirts you can often tollerate significant clipping since only the outermost layer has to hold." The earlier lean toward "just shrinking the body colliders" is this rule's body half taken alone; its other half, that thinning chains lets legs slip between them, is the chain half.
   - **Add the colliders the region needs where none exist**: hips plus both upper legs for a skirt, chest plus shoulders for hair. Size for the measured effect, not the skin: a capsule sized to the body surface leaves residual overlap because collision resolution is partial per solver step, and the collider that fixed the source case was thinner and taller than the geometry, which worked because the skirt's chain colliders were very large (the pairing above). It may touch a chain at rest, but every touch costs an inflate on play entry, the chain the editor shows pushed out the moment the solver starts, so keep each chain's `restContactCm=` to a touch and never a major deflection (`runtime.md` §PhysBones), with no chain printing `—` for a collider it should be measured against. Contact must land well below the chain root, since a touch near the root levers the hem. Capsule `height` is the core segment; the rounded ends add two radii, and the report prints `endToEnd` beside `height` so the two are never confused (the SDK's contact shapes count their caps inside `height`; physbone colliders do not).
