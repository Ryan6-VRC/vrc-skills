...
**Zero the instance's root transform before saving — unless the root carries a fit you established here.** Phase 1 replicated any vendor root delta into the geometry, so on an armature-merge piece the root holds nothing but staging offset, which bakes into the asset and surfaces later as a `CheckSeam` `maxOffset` fail. The exception is a bone-proxy piece whose placement was set here — a fit transferred from a vendor's per-base variant, say: Phase 2B authors `AsChildKeepWorldPose`, whose build preserves the edit-time world pose, so that root value *is* the fit and zeroing it moves the piece. Read `attachmentMode` before deciding — under `AsChildAtRoot` or `Unset` the build zeroes localPosition and localRotation itself, so the root is never the fit there. Record which case you have; `compose-mergeable`'s never-normalize rule protects a kept root downstream. Hand to `compose-mergeable` + the operator's playmode for the visual/behavioral bar. A `RenderAvatar` grab for that bar goes in a separate call from any edit — a same-call grab shows the pre-edit proxy; the summary's `note=` flags an in-flight rebuild but cannot catch the same-call case.
