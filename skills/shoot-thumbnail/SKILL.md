...
- **`head=` is the only in-frame report there is, and it covers only the head.** Nothing in the verdict says whether a prop, accessory, or held object landed in the frustum, so an object absent from the PNG is not evidence it failed to render. Before blaming the bake, the shader, or the camera for one, park a duplicate of it on the Head bone and re-shoot: if it draws there, the shot was framed past it. A `bust` crop sits tight on a head at the avatar root and a hand-height prop is half a metre outside that, which makes framing both the likelier explanation and the cheap one to rule out first — chase a render cause only once this test has cleared it.
