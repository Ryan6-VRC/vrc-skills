...
- **Picking a vendor-shipped colorway** (vendors ship whole PNG/JPG variant sets and parallel color `.mat`s) is assignment, not owning — swap the reference (`RemapMaterials`, contracted in `unity-tools.md`, or the renderer slot in-scene) and stop. It enters this skill only when the chosen variant then gets edited.
