...
- A **`remapUNRESOLVED`** FAIL is the other half and takes the **opposite order**: the remap points at materials that do not exist, so the pack was never imported. Import it *first*, then force-reimport. Reimporting on its own is a guaranteed no-op here, and repeating it looks like a stuck tool rather than a missing package. Don't identify either class by reading `materialLocation` — a modern remap reads `InPrefab` (`unity.md` §Sharp edges).
