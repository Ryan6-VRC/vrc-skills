...
- **Humanoid-bone coincidence before any render**: compare instance-vs-target-body `Animator.GetBoneTransform` world positions by bone name. **Gate: offsets at solver-noise scale (≤ 0.01 mm over the shared humanoid bones; the verified run measured ~0.001 mm)** — millimetre-scale is a wrong result, not slop. Over the gate → the likeliest cause is step 3's two-fields trap: don't hand off, re-confirm the route against the window-derived config, re-fire, re-own. `CheckSeam` refuses the raw pairing (`docs/mochifitter.md`); it scores later, placed on the real base at compose.
