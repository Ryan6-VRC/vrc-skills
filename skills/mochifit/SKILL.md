...
- **Humanoid-bone coincidence before any render**: `CheckSeam.CheckBare(targetBody, instance, maxOffsetMm: 0.01f)` — the pre-seam door (`docs/unity-tools.md`), which pairs by bone name because the raw output's `MergeArmature` has no base to resolve against yet and `CheckSeam.Check` rightly abstains. **The tolerance is this skill's to state, and 0.01 mm is it**: solver-noise scale, the verified run measuring ~0.001 mm — millimetre-scale is a wrong result, not slop. `NOT-PASS` → the likeliest cause is step 3's two-fields trap: don't hand off, re-confirm the route against the window-derived config, re-fire, re-own. `Check` scores later, placed on the real base at compose.
