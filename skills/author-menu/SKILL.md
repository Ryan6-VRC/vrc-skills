...
- **A `MenuItem` added from code does not get the inspector's defaults.** `ModularAvatarMenuItem.InitSettings()` is `internal` and unreachable from `execute_code`, so a bare `AddComponent` leaves `Control` null and two defaults wrong: `MenuSource` starts at `MenuAsset` rather than `Children` (which bites on a SubMenu node, not a leaf toggle), and `automaticValue` starts `false`. Set what your plan depends on explicitly — MA's own fields and properties are public, so assign them directly.
...
- **Unity MCP `execute_code`** — all authoring and reading on the scene instance. No dedicated menu tool exists, deliberately — the deterministic slice is small and the formats are moving targets.
