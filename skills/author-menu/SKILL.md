...
- **A `MenuItem` added from code does not get the inspector's defaults.** `ModularAvatarMenuItem.InitSettings()` is `internal`, and MA's `Reset()` deliberately skips it on a scripted add, so a bare `AddComponent` leaves `Control` **blank rather than null** — `OnValidate` fills it with an invalid `type` of 0, so a null test reads clean — plus two defaults wrong: `MenuSource` starts at `MenuAsset` rather than `Children` (which bites on a SubMenu node, not a leaf toggle), and `automaticValue` starts `false`. Set what your plan depends on explicitly, or reflect `InitSettings()` as MA's own shortcuts do.
