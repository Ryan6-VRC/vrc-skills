...
- **`ModularAvatarMenuItem.InitSettings()` is `internal`** — a fresh `MenuItem` added from `execute_code` must set every field explicitly via `SerializedObject`; the method the component's own inspector calls to initialize defaults is not reachable.
