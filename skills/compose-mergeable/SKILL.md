...
Instantiate the **standalone vendor/owned prefab** (never a placed instance) as a child of the avatar root, **leaving the prefab's own root transform untouched — untouched, not identity**: vendors bake the per-avatar fit into the prefab root (a real hair shipped root scale `1.051` plus a position offset; zeroing them to identity seated the whole assembly 6.5 cm low while every mechanical gate passed). The root transform is authoring, not mess — **never normalize its scale or position** as cleanup: normalizing a vendor root scale like `0.9512` to `1:1:1` after placement manufactures a per-bone offset gradient that reads as a gross misfit and bakes straight through (the merge is identity-preserving — `nondestructive.md`). **After the drop, before any operator-directed placement, compare the instance root's local position/rotation/scale values against the prefab asset's stored values** — a value that differs from what the prefab authored is an unintended override; revert it, don't re-author it. The check is a **value equality test**, not a modification-count diff: Unity always records root pos/rot as property modifications on a fresh instance under a new parent, so the presence of override entries is normal — only a **changed value** signals drift. A later deliberate, flagged positioning of a bone-proxy mergeable is the operator's (step 3's proxy REFUSE route) and is not what this check reverts.

**Drop parent matters for BoneProxy KeepWorldPose.** A `MergeArmature` seam is parent-agnostic — it finds bones by name and the drop parent is irrelevant. A `BoneProxy` with `AsChildKeepWorldPose` is **parent-determined**: the build preserves the edit-time world position (`BoneProxyProcessor` re-parents then restores world pos/rot), so the same untouched local numbers under Head vs. the avatar root differ by the parent's world offset. Drop a BoneProxy mergeable as a **child of the avatar root** (the default) unless the vendor or `own-mergeable` documents a different parent — and if you move it, understand that the moved world position is the one that ships. A mergeable authored for this base auto-targets the base's `Armature` — you do not wire the seam by hand.
...
**Gimmick/behavior modules skip this entire step** — a whole module replaces nothing and keeps all its layers, so it has no de-conflict obligation, no delegation, and no checkpoint artifact from this step. A *partial* take is an `own-gimmick` fork, not a compose. Everything below is the geometry case (clothing, hair, ears — any mergeable that supersedes a base-feature layer).

A composed mergeable that supersedes a base-feature layer (clothing under an outfit, hair under replacement hair, ears under replacement ears) creates overlap; the superseded meshes clip. Disabling those meshes looks like a one-line in-scene edit and is not. A base layer is often **coupled to body blendshapes** (`outfits.md`): disable the mesh alone and the pre-collapsed region it covered stays collapsed. The coupling lives in FX clips and MA `ShapeChanger` reactions, on a **body-morph mesh that is not the one named `Body`**, and reads **weight 0 at edit time** — invisible to any scan you would run here. Deciding which layers are actually overlapped, ruling on each shape, and gathering the evidence for a contested keep is `map-outfit-shapes`' domain, not this skill's.

So whenever this step disables or deletes a superseded base-feature layer, **delegate to `map-outfit-shapes`, scoped to the composed mergeable and the base layers it could replace** (the full-avatar map stays opt-in) — naming which of those are actually overlapped is map's call, not this step's — and require two artifacts back before the geometry-path compose is done. A gimmick module produces neither (it skips this step entirely):
