...
1. its **`ReportShapeOverlap` resolution artifact**, every `MISMATCH` row accepted, overridden, or left explicitly **OPEN**, whose RunLog records the census ran; and
...
| "I scanned the FX myself — no delegation needed" | A clean scan is the map's *empty outcome*, not a waiver of the map: the same read done inside the delegation produces the artifact and the `RESIDUE: none` line. A scan that leaves neither artifact nor `MAP SKIPPED:` line is the skip, however real the scan was. |
...
- **Suppressing an unwanted piece of the mergeable itself** (the vendor ships style variants or optional accessories active — extra ears, a selectable twintail style): an MA **parameter default is not a disarm** — the always-on-layer doctrine strikes through param defaults too, and a statically-active mesh ships active regardless of what the default says. Suppress with a static disable, or a toggle you verified drives the mesh; then check each piece's `activeInHierarchy` on the built/play clone against intent, including the vendor's style selectors (they bake to *some* default state, not necessarily a coherent one). The mechanical seam gates say nothing about this — they certified a double-eared compose as fine.
...
A body morph (breast size, a proportion tweak) is a **same-set coherence value**: every mergeable on the base must reach it — **one value per morph**, the invariant whose authority is `reproportion` (*Realizing shapekeys*); honor it there, don't re-derive it. The value lives in one of two forms:
