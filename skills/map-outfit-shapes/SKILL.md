...
**Reading a `ShapeChanger` raw:** prefer reflection over the `m_shapes` field (the serialized array is `m_shapes`, not `Shapes`) — it prints the `ShapeChangeType` by *name* (`Set`/`Delete`), which settles the mode outright. A SerializedProperty read gives the enum index instead — **Delete=0, Set=1** — so a bare `enum[0]` is *Delete*, the mode that hides the vertices its shape selects (`outfits.md` owns what that costs), the more consequential mode, not the harmless-looking default; resolve the name before acting on it.
...
Pass the body-morph mesh **and the outfit root** — reaction ingestion is gated on that second argument. On a mixed compose (two outfit sources on one avatar), pass the **avatar root** instead — per-source runs each manufacture MISMATCH rows the other root's declarations would have resolved (`unity-tools.md`). Omit it and the analysed set silently narrows to caller-passed ∪ worn-nonzero, dropping **exactly the weight-0 reactions** the census exists to surface. **A zero reaction count settles nothing on its own — the summary says which of its two readings applies**: it distinguishes the omission above from a scan that ran and found no reaction reaching this mesh, and in that second case names whether the outfit carries mesh cutters instead, so the zero is genuine because the outfit cuts rather than reacts. Rows aimed at a sibling mesh are filtered out of the table, so their absence is not drift.
...
**An inactive-ancestor annotation on a reaction cell is a second signal, and it is not a flag.** Every declaration of that shape sits behind a parked ancestor, so it may not apply at the initial state — the tool cannot settle it, because proving a declaration dead needs the build's animation index. Read the named ancestor: dead authoring residue means the shape is effectively undeclared and takes the ordinary presumption below; an ancestor that *should* be live (or wants a toggle it never got) makes the ancestor the repair, not the weight. Either way you are ruling on structure, so **discharge it by the structural call, not by a `CaptureDiff` differential** — tag the row `stranded-declaration` and name the ancestor. Toggle-driven, `Inverted`, and live-sibling declarations never carry this annotation.

Two cells are **not dispositionable**: two reactions declaring different `(type, value)` for one shape, and an unmodelled `ShapeChangeType`. Neither has a presumption to discharge — **surface it, never pick**. The first is the merge case above (two kept pieces driving one body shape to different values) caught mechanically.

A name in the analysed set that isn't on the mesh is reported missing, and the rest still analyse. When it's a *reaction-targeted* name, that is the finding: a `ShapeChanger` still pointing at a renamed or deleted shape.
...
**Each MISMATCH row's resolved-target is a defeasible presumption — discharge it, never eyeball it.** A worn-but-undeclared shape resolves toward its target unless you **override with a named `CaptureDiff` differential** showing the target value defects — the base foot piercing the outfit sole, a gap, a clip over the region. **A render *look* is never override currency** (it reads clean over a real clip). A **fully-occluded** shape (a shrink cage, by definition) structurally cannot produce a differential — there, a `BakeMesh` geometry/penetration probe at the candidate weights is equal override currency (`verify.md` sanctions it as the pixel-independent instrument); bake the **NDMF preview proxy**, not the source mesh — the proxy carries the resolved reactive state, and `verify.md` names the proxy-vs-source split and the attribution call. Accept or override, both logged; an un-dispositioned MISMATCH stays **OPEN**. It cuts both ways: a footwear outfit that declares no foot-pose shape (`Heel_Feet`/`Foot_heel`) resolves it to 0, releasing a base heel a scan left worn — the observed `Heel_Feet=100`-kept-on-a-render failure, not a constructed illustration; a heeled outfit that *forgot* the declaration also resolves to 0, overridden back to 100 only when a `CaptureDiff` at 0 shows the flat foot piercing the sole.

**A MISMATCH row can be a shared-morph edge, not a coupling edge — the presumption doesn't apply there.** A worn-but-undeclared shape that is a vendor-authored *body* shape (a breast-size or proportion morph, e.g. worn `Breasts_small=75`) is the map's shared-morph edge wearing a MISMATCH flag: the resolved-target presumption is a *coupling* presumption with no purchase here — no garment drives the morph, so resolving it to 0 releases no coupling; it rewrites authored body proportions and breaks the one-value-per-morph obligation §Shape coherence hands to `compose-mergeable` §5. Its disposition is **keep-as-authored**, logged as an **override discharged by the category call rather than by a `CaptureDiff` differential** — tag the row `shared-morph` in the resolution artifact.
