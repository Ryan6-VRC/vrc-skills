...
**`MISMATCH` is `worn ∧ not-live-declared`, never `current ≠ resolved-target`.** A row a *live* reaction declares is never flagged however far its edit-time weight sits from its target, because the reaction owns it at runtime; reading the column as a diff manufactures offenders the build resolves on its own.

**Live is the operative word — a declaration from behind a parked ancestor owns nothing.** MA conditions every reaction on its ancestors' active state, so a `ShapeChanger` under an inactive object that nothing toggles does not apply at the initial state. Those rows carry `[inactive: <path>]`, count in `inactiveDeclared=`, and flag like any undeclared row. An outfit merely switched off in the scene but driven by an `ObjectToggle` is live and still declares, so this is not the ordinary toggled-outfit case — it is a declaration stranded under something parked.
...
**A MISMATCH row can be a stranded declaration, and that one is settled structurally.** A row carrying `[inactive: <path>]` is flagged because the only thing declaring it sits under a parked object — so the question is not what the shape should resolve to but whether that object was meant to be parked. Read the named ancestor: if it is dead authoring residue, the row is a genuine worn-and-undeclared shape and takes the ordinary presumption; if it should be live (or driven by a toggle that is missing), the repair is the ancestor, not the weight. Either way the disposition is **discharged by the structural call, not by a `CaptureDiff` differential** — tag the row `stranded-declaration` and name the ancestor. Demanding a pixel differential here would price a routine reading like a coupling argument and prove nothing about the cause.
