...
**A region the census shows neither covered nor cut is repaired on the covering garment, not on the base.** Declare the region's rows on that garment's own prefab (`outfits.md`) — a base-side cutter or rest weight makes the hide un-overridable by the next outfit, which is the defect the per-garment declaration exists to avoid.
